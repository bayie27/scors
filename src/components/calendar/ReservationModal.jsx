          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="border rounded px-3 py-2 bg-gray-100">
                {(() => {
                  if (statuses && statuses.length > 0 && form.reservation_status_id) {
                    const statusObj = statuses.find(s => s.reservation_status_id == form.reservation_status_id);
                    return statusObj ? statusObj.name : `ID ${form.reservation_status_id}`;
                  }
                  return form.reservation_status_id ? `ID ${form.reservation_status_id}` : 'Unknown';
                })()}
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium">Created (Reservation TS)</label>
              <div className="border rounded px-3 py-2 bg-gray-100">{form.reservation_ts ? new Date(form.reservation_ts).toLocaleString() : 'N/A'}</div>
            </div>
            <div>
              <label className="block text-sm font-medium">Last Edited (Edit TS)</label>
              <div className="border rounded px-3 py-2 bg-gray-100">{form.edit_ts ? new Date(form.edit_ts).toLocaleString() : 'N/A'}</div>
            </div>
            <div>
              <label className="block text-sm font-medium">Purpose</label>
              <div className="border rounded px-3 py-2 bg-gray-100">{form.purpose}</div>
            </div>
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="block text-sm font-medium">Date</label>
                <div className="border rounded px-3 py-2 bg-gray-100">{form.activity_date}</div>
              </div>
              <div className="flex-1">
                <label className="block text-sm font-medium">Start Time</label>
                <div className="border rounded px-3 py-2 bg-gray-100">{form.start_time}</div>
              </div>
              <div className="flex-1">
                <label className="block text-sm font-medium">End Time</label>
                <div className="border rounded px-3 py-2 bg-gray-100">{form.end_time}</div>
              </div>
            </div>
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="block text-sm font-medium">Venue</label>
                <div className="border rounded px-3 py-2 bg-gray-100">{venues.find(v => v.venue_id == form.venue_id)?.venue_name || 'None'}</div>
              </div>
              <div className="flex-1">
                <label className="block text-sm font-medium">Equipment</label>
                <div className="border rounded px-3 py-2 bg-gray-100">{equipmentList.find(e => e.equipment_id == form.equipment_id)?.equipment_name || 'None'}</div>
              </div>
            </div>
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="block text-sm font-medium">Reserved By</label>
                <div className="border rounded px-3 py-2 bg-gray-100">{form.reserved_by}</div>
              </div>
              <div className="flex-1">
                <label className="block text-sm font-medium">Officer in Charge</label>
                <div className="border rounded px-3 py-2 bg-gray-100">{form.officer_in_charge}</div>
              </div>
              <div className="flex-1">
                <label className="block text-sm font-medium">Contact No.</label>
                <div className="border rounded px-3 py-2 bg-gray-100">{form.contact_no}</div>
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <button type="button" onClick={onEditView} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Edit</button>
            </div>
          </div>
        ) : (
